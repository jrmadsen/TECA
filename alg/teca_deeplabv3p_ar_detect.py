...
class ResNet(nn.Module):
    ...
    def _load_pretrained_model(self):
        state_dict = torch.load("resnet101-5d3b4d8f-state_dict.pth", map_location=lambda storage, loc: storage)
...
def transform_shape(image, shape):
...
class teca_deeplabv3p_ar_detect(teca_py.teca_model_segmentation):
    ...
    def __init__(self):
        ...
        print(" teca_py.teca_get_data_root():", teca_py.teca_get_data_root())
        state_dict_path = teca_py.teca_get_data_root() + self.state_dict_filename
        print("state_dict_path:", state_dict_path)

        model = teca_py.teca_load_state_dict(state_dict_path,
            DeepLabv3_plus, n_classes=1, pretrained=True)

        self.set_model(model)

        self.set_transform_fn(transform_shape, [1, 3, 384, 576])
